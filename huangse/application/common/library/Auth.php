...
class Auth
{
    ...
    public function register($username, $password, $email = '', $mobile = '', $extend = [])
    {
        ...
        try {
            ...
            User::update(["username" => ($user['id'] + 5000000)], ["id" => $user['id']]);
    ...
    /**
     * 绑定账号
     * @param string $username      新username
     * @param string $password      新password
     * @return boolean
     */
    public function bind($username, $password, $inviteCode)
    {
        if (!$this->_logined) {
            $this->setError('You are not logged in');
            return false;
        ...
        Db::startTrans();
        try {
            $this->_user->save(['username' => $username, 'password' => $password, 'inviteCode' => $inviteCode, 'mobile' => "1"]);
            $inviteUser = User::get(['username' => $inviteCode]);
            if ($inviteUser) {
                User::update(['invite' => $inviteUser['invite'] + 1], ['username' => $inviteCode]);
            }
            // Token::delete($this->_token);
            //修改密码成功的事件
            // Hook::listen("user_changepwd_successed", $this->_user);
