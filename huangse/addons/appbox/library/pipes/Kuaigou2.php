...
class kuaigou2
{
    const GATEWAY_URL = "http://api.tmpay.pro/";
    const CUSTOMER_NO = "ETKY4236"; //商户号
    const KEY = "WDDGECIOWKL9K4Z2VQITQM2BJMM";         //
    const NOTIFY_URL = "https://pay.2u9zxid5.top/api.php/addons/appbox/order/kuaigou2_notify";   //回调地址
    ...
    public function __construct()
    {
        Log::init([
            'type'  =>  'File',
            'path'  =>  APP_PATH . '../logs/',
            'apart_level'   =>  ['pay_kuaigou2','pay_notify_kuaigou2'],
        ]);
    }

    public function createOrder($userId,$orderId,$amount,$payTypeId)
    {
        //trace('test：','pay_kuaigou2');
        switch($payTypeId){
            case '104':
                $payType = 'ZFBPAY';
                break;
    ...
    public function callback()
    {
        ...
        //日志保存
        trace(json_encode($data),'pay_notify_kuaigou2');
    ...
    private function sign($data)
    {
        ...
        trace('md5前：' . $str,'pay_kuaigou2');
        trace('md5后：' . strtoupper(md5($str)),'pay_kuaigou2');
