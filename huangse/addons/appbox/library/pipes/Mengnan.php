...
class Mengnan
{
    const GATEWAY_URL = "https://tks6.tczajy.com";
    const CUSTOMER_NO = "322"; //商户号
    const KEY = "92984530473B11EFA25E4F2ECC667985";         //
    const NOTIFY_URL = "https://pay.2u9zxid5.top/api.php/addons/appbox/order/mn_notify";   //回调地址
    ...
    public function __construct()
    {
        Log::init([
            'type'  =>  'File',
            'path'  =>  APP_PATH . '../logs/',
            'apart_level'   =>  ['pay_mengnan','pay_notify_mengnan'],
        ]);
    }
    ...
    public function callback()
    {
        ...
        if(isset($data['attach'])){
            $signData['attach'] = $data['attach'];
        }


        //日志保存
        trace(json_encode($data),'pay_notify_mengnan');
    ...
    private function sign($data)
    {
        ...
        trace('md5前：' . $str,'pay_mengnan');
        trace('md5后：' . strtoupper(md5($str)),'pay_mengnan');
